...
| 能力 | 说明 |
| --- | --- |
| 知识库与 RAG 问答 | 文档上传解析（Tika）→ 分块 → 向量入库；混合检索（向量 0.7 + 关键词 0.3）+ 查询路由 + 双模式 Rerank（可回退）；多轮会话（查询改写）、引用溯源、版本对比回滚 |
| Agent 编排 | ReAct + Plan-Execute 双模式，工具调用校验/超时/审计，**统一输入防护（Prompt 注入检测）**，长期记忆（分层：核心常驻 + 场景按需注入） |
| 接口工具化 | **自研 OpenAPI 3.0 解析器**（Jackson + SnakeYAML）批量导入接口 → pgvector 语义档案 → 自然语言检索命中 → 对话/工作流按需注入（超 20 个注入 top 8） |
| 缺失能力反推 | 自然语言需求 → LLM 拆解 → 语义检索标注覆盖/缺口 → 生成 OpenAPI 补全片段回流接入；**模型编造的接口白名单校验降级** |
| 工作流引擎 | 自然语言生成工作流（顺序/条件分支/并行组/**循环** + 变量传递）；**同工作流并发触发排队串行**（per-workflow 闸门）；**分级重试 + 断点续跑**（复用步骤记录） |
| 技能沉淀复用 | 工作流沉淀为技能时自动记录接口依赖 → 新会话命中技能按声明注入接口（不依赖语义检索，稳定可复现） |
| SQL 诊断 | EXPLAIN 解析 + 规则初筛 + AI 建议（失败回退规则），支持 mock 模式 |
| 模型韧性治理 | 多级备援（智谱→SiliconFlow→OpenRouter）、**简单任务走便宜档分级路由**（chatCheap）、429 分级熔断 60s、超时快失败（5s/15s）、错误分类按 HTTP 状态码、**用量配额拦截（日次数/日成本）**、本地 RAG 兜底、Embedding 两级缓存（内存 + 表） |
| 平台治理 | 多租户团队隔离、登录/安全加固（可关）、审计日志、50 条检索评估管线（MRR/Recall@K/NDCG/**Faithfulness** + 基线护栏）、**统一有界线程池**、Prometheus 监控 |
...
- **测试**：后端 **402** 单测/集成（含越权/注入防护/模型降级/并发排队/Agent 组件/工作流 Loop 与断点/记忆分层/Faithfulness）+ 前端 40 vitest + Playwright E2E
...
- **检索评估**：50 条样本离线评估管线（MRR / Recall@K / NDCG / **Faithfulness**），检索/生成质量回退 >5% 自动告警
